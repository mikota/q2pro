...
void G_LoadLocations( void )
{
	...
	if (!*game_cvar->string)
		Q_snprintf(locfile, sizeof(locfile), "%s/tng/%s.aqg", GAMEVERSION, level.mapname);
	else
		Q_snprintf(locfile, sizeof(locfile), "%s/tng/%s.aqg", game_cvar->string, level.mapname);
...
int LoadFlagsFromFile (const char *mapname)
{
	...
	Q_snprintf(buf, sizeof(buf), "%s/tng/%s.flg", GAMEVERSION, mapname);
	fp = fopen(buf, "r");
	if (!fp)  {
